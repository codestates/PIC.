...
function getIncluded(latX, lonX, latY, lonY, dist) {
    ...
    if (Math.abs(latX - latY) * D > dist || Math.abs(lonX - lonY) * C > dist) return false;
    ...
    class DMS {
        constructor(coordinate) {
            ...
            this.S = Math.round(((((coordinate - this.D) * 60) - this.M) * 60) * 100) / 100;
        }
    }
    const [X1, Y1, X2, Y2] = [new DMS(latX), new DMS(lonX), new DMS(latY), new DMS(lonY)];
    const X = {
        D: X1.D - X2.D,
        M: X1.M - X2.M,
    ...
    // 두점 사이의 거리 구하는 공식
    const distance = Math.sqrt(((X.D * D) + (X.M * D / 60) + (X.S * D / 3600))**2 + ((Y.D * C) + (Y.M * C / 60) + (Y.S * C / 3600))**2);
